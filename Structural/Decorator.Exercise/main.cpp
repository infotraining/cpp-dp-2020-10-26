...
int main()
{
    ...
    auto cf2 = std::make_unique<Whisky>(std::move(cf));
    auto cf3 = std::make_unique<Whipped>(std::move(cf2));
    auto cf4 = std::make_unique<Whisky>(std::move(cf3));    
    client(std::move(cf4));

    client(std::make_unique<Whisky>(std::make_unique<Cappuccino>()));
